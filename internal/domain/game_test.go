...
func TestCalculateKillPointsForMultipleKillByPlayers(t *testing.T) {
	...
	if g.Kills["player2"] != 1 {
		t.Errorf("CalculateKillPoints().Kills[player1] = %d, want 1", g.Kills["player2"])
	}
	if g.Kills["player3"] != 0 {
		t.Errorf("CalculateKillPoints().Kills[player1] = %d, want 0", g.Kills["player3"])
	}
	if g.Kills["<world>"] != 0 {
		t.Errorf("CalculateKillPoints().Kills[player1] = %d, want 0", g.Kills["<world>"])
	}
